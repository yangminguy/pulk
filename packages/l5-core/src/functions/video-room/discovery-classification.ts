...
function buildSystemPrompt(mode: DiscoveryMode): string {
  ...
  if (mode === 'key') {
    lines.push(
      '[키 콘텐츠 판단 기준]',
      '① 시청자 정체성이 같은가 (최우선) — 이 영상의 제목·주제가 실제로 끌어당기는 "시청자의 정체성·자기인식 레벨"이 우리 핵심 타깃과 같은지. 주제가 카테고리상 관련 있어도, 그 영상을 클릭·시청할 사람이 우리 타깃보다 낮은 작업·실무 단위 정체성이면(예: 우리 타깃은 "대표/사업가"인데 영상은 "OO를 직접 하고 싶은 실무자/작업자"를 끌어당김) 정체성 불일치 → unfit. 조회수·성과가 높아도 이 기준이 어긋나면 fit으로 보지 마라.',
      '② 판매논리가 녹는가 — 문제 제기 → 카테고리의 기능/특징/장점(FBB) → 카테고리 필요성 → 아이템 FBB로 해결 → 아이템 제안으로 이어지는 판매 논리를 이 주제에 자연스럽게 녹일 수 있는지.',
      '①을 충족하고 ②도 충족하면 fit, ①이 어긋나면(시청자 정체성 불일치) unfit, 판단이 갈리면 ambiguous.',
    );
  } else {
    lines.push(
      '[풀링 콘텐츠 판단 기준]',
      '① 같은 시청자 정체성인가 (최우선) — 키 콘텐츠와 동일한 타깃이, 동일한 정체성·자기인식 레벨에서 보는 주제인지. 주제가 관련 있어도 시청자가 우리 타깃보다 낮은 작업·실무 단위 정체성을 가지면(대표 타깃인데 실무자/작업자를 끌어당기는 주제) 불일치 → unfit. 조회수가 높아도 이 기준이 어긋나면 fit 금지.',
