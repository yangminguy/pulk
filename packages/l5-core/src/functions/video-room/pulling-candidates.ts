...
function buildPrompt(input: PullingCandidateInput, count: number): string {
  ...
  return [
    ...
    '★ 최우선 — 시청자 정체성 정합성: 각 풀링 주제는 키 콘텐츠와 동일한 "시청자 정체성·자기인식 레벨"을 가진 사람이 볼 주제여야 한다. 주제가 카테고리상 관련 있어도, 그 주제가 끌어당기는 시청자가 키 타깃보다 낮은 작업·실무 단위 정체성이면(키 타깃이 "대표/사업가"인데 "OO를 직접 하고 싶은 실무자/작업자"를 끌어당김) 제외하라. 조회수가 잘 나올 법해도 정체성이 어긋나면 만들지 마라.',
