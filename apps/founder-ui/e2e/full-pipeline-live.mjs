...
const STATE_FILE = '/tmp/e2e-pipeline-state.json'
...
async function main() {
  ...
  if (!done('create')) {
    const d = must(await call('createProject', {
      title: 'E2E 라이브 파이프라인 — AI 마케팅팀 (2026-06-11)',
      product: 'AI 마케팅 자동화 팀',
      target_audience: '마케팅팀을 둘 여력이 없는 작은 브랜드 대표',
      business_goal: 'consulting_lead',
      customer_problem: '콘텐츠를 올려도 손님이 오지 않고, 뭘 고쳐야 할지 모른다',
      core_offer: 'AI 마케팅팀 진단 신청',
    }), '1. createProject')
  ...
  if (!done('matrix')) {
    const d = must(await callOrPoll('proposeThumbnailMatrix', {
      ...
      title: state.selected_title ?? 'AI 마케팅팀',
      main_click_reason: '내 콘텐츠가 매출로 안 이어지는 진짜 이유를 확인하고 싶다',
      channel_audience_profile: '마케팅팀 없이 직접 콘텐츠를 만드는 작은 브랜드 대표',
  ...
  // 13. 승인④ → 녹음 (say로 실음성 생성)
  if (!done('voice')) {
    await advanceUntil('voice_recording')
    try { execSync(`say -o /tmp/e2e-voice.aiff "안녕하세요. 이 영상은 전체 파이프라인 검증용 테스트 녹음입니다."`, { timeout: 30000 }) } catch { /* 음성 합성 실패해도 file_url만 필요 */ }
    const d = must(await call('attachVoice', { project_id: state.project_id, file_url: 'file:///tmp/e2e-voice.aiff', duration_sec: 8 }, 60000), '13. attachVoice')
    log(`   status → ${d.status}`)
    mark('voice')
  }
  ...
  if (!done('uploaddraft')) {
    ...
    const d = must(await call('createUploadDraft', { project_id: state.project_id, render_job_id: state.render_job_id, title: state.selected_title ?? 'E2E 영상' }, 300000), '17. createUploadDraft')
