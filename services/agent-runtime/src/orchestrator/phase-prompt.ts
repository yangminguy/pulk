...
const HEADER = [
  ...
  '',
  '★ 너의 작업 디렉토리(cwd)는 아래 표시된 격리 worktree다. 모든 파일 경로·명령은',
  '이 cwd 기준(상대경로/이 cwd 안)으로 하라. 메인 레포 절대경로(~/Desktop/pulk 등)를',
  '직접 가리키지 마라 — 격리가 깨진다. tsc/jest도 이 worktree 안에서 실행하라.',
  '',
